...
function isWellFormed(element) {
  if (
    ...
    ("DESC" in element[0][Object.keys(element[0])[0]])  && //description key exists
    ("NAICS" in element[0][Object.keys(element[0])[0]])  && //NAICS key exists
    ("CLASSCOD" in element[0][Object.keys(element[0])[0]])  && //CLASSCOD key exists
    ("SUBJECT" in element[0][Object.keys(element[0])[0]])  && //SUBJECT key exists
    ((Object.keys(element[0])[0] == 'AWARD') || ("SETASIDE" in element[0][Object.keys(element[0])[0]])) //SETASIDE key exists unless type 'AWARD'
    ) { //well-formed opportunity record
      return true;
    } else { //not well-formed opportunity record, flag so I can follow up
      console.log('Poorly formed opportunity record, eliminated.');
      return false;
    };
};
...
function isInteresting(element) {
  ...
  const myDealType = 'PRESOL SRCSGT SNOTE COMBINE AMDCSS MOD AWARD';
  ...
  //Check whether the JSON record from FBO is well formed and opportunity is interesting  //data[1][0][Object.keys(data[1][0])[0]]["DESC"]
 if ((isWellFormed(element)) &&
    ((myDealType.match(Object.keys(element[0])[0]) != null) && // opportunity type I care about
    (myNaicsStr.match(element[0][Object.keys(element[0])[0]]['NAICS']) != null) && // NAICS code I care about
    (myClassStr.match(element[0][Object.keys(element[0])[0]]['CLASSCOD']) != null) && // Class code I care about
    !(soleSource.test(element[0][Object.keys(element[0])[0]]['DESC'])) && // not described as sole source
...
// For loop over all files (assumes I have every day in the date range), process and then add output array to master to be written to the output files
  ...
  myCorpus[Object.keys(element[0])[0]] += element[0][Object.keys(element[0])[0]]['SUBJECT'] + '\n' + element[0][Object.keys(element[0])[0]]['DESC'] + '\n';
    ...
    fs.writeFileSync('Output Files/Matt_Test_PRESOL.json', JSON.stringify(myCorpus.PRESOL));
    fs.writeFileSync('Output Files/Matt_Test_SRCSGT.json', JSON.stringify(myCorpus.SRCSGT));
    fs.writeFileSync('Output Files/Matt_Test_SNOTE.json', JSON.stringify(myCorpus.SNOTE));
    fs.writeFileSync('Output Files/Matt_Test_COMBINE.json', JSON.stringify(myCorpus.COMBINE));
    fs.writeFileSync('Output Files/Matt_Test_AMDCSS.json', JSON.stringify(myCorpus.AMDCSS));
    fs.writeFileSync('Output Files/Matt_Test_MOD.json', JSON.stringify(myCorpus.MOD));
    fs.writeFileSync('Output Files/Matt_Test_AWARD.json', JSON.stringify(myCorpus.AWARD));
